...
- Some participants took the opportunity to verify PGP fingerprints and sign each others' keys, to expand the PGP web of trust around Ion for improved security.
- The special travis caching features that Ion Core is using should now available to all GitHub users, so the CI tests should run on arbitrary repositories, and not just the Ion Core one.
...
The Ion network does not encrypt communication between peers today. This opens up security issues (eg: traffic manipulation by others) and allows for mass surveillance / analysis of ion users. Mostly this is negligible because of the nature of Ions trust model, however for SPV nodes this can have significant privacy impacts and could reduce the censorship-resistance of a peer.

Encrypting peer traffic will make analysis and specific user targeting much more difficult than it currently is. Today it's trivial for a network provider or any other men-in-the-middle to identify a Ion user and its controlled addresses/keys (and link with his Google profile, etc.). Just created and broadcasted transactions will reveal the amount and the payee to the network provider.

The protocol used to communicate between Ion nodes has always been unencrypted, since communications are considered public. However, there is desire to have the option of security for users who run thin clients for their wallet, yet want to use their own private node for security.
...
Ion Core's networking code is very minimal and not very flexible or easy to improve on. Cory has been working on rewriting it.
...
[BIP114][], Merkelized Abstract Syntax Tree (MAST), is an enhancement of the Ion scripting language that takes advantage of segwit script versioning. It increases efficiency and privacy of conditional transactions. This [BIP114][] also safely enables many of the opcodes that were disabled in an early version of Ion. MAST scripts increase privacy because less data needs to be made public, and can also make transactions smaller, thus saving space.
...
Review the following pull-requests [\#7600](https://github.com/cevap/ion/pull/7600) and [\#7598](https://github.com/cevap/ion/pull/7598).
