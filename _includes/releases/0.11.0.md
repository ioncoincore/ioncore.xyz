...
# Text originally from Ion Core project
# Metadata and small formatting changes from Ion.org project
...
## changes to the release notes from the Ion Core source tree:
##
...
##
## 2. Place URLs inside angle brackets, like <http://ion.org/bin>

---
{% githubify https://github.com/ion/ion %}
Ion Core version 0.11.0 is now available from:

  <https://ion.org/bin/ion-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer ION/kB fee to be
...
For example, add the following to `ion.conf`:
...
related to the blockchain in the ion system: the raw blocks as received over
the network (blk???.dat), the undo data (rev???.dat), the block index and the
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Ion Core to delete the raw block and undo data once
...
block index and UTXO databases. The minimum was chosen so that Ion Core will
...
For an example script see [ion-submittx](https://github.com/laanwj/ion-submittx).
...
- #5334 `cf87536` libionconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new ion-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only IonAmountField
- #5753 `0f44672` Add ion logo to about screen
...
- #5809 `46bfbe7` Add ion-cli man page
...
- #5849 `41259ca` contrib/init/iond.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Ion-Qt.app to Ion-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for ion-tx)
